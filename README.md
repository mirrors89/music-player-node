...
- 모든 조회 API는 오늘 날짜(한국 시간 기준)의 곡만 반환합니다
- 매일 자정(한국 시간 00:00)이 지나면 자동으로 새로운 플레이리스트로 초기화됩니다
- 과거 데이터는 데이터베이스에 그대로 남아있어 필요시 조회 가능합니다
- `play_order`는 매일 1부터 다시 시작됩니다
- **Asia/Seoul 시간대(UTC+9) 기준으로 작동**

### 동작 방식

```sql
-- 예시: 오늘 날짜의 미재생 곡만 조회 (한국 시간 기준)
SELECT * FROM songs
WHERE is_played = 0
...
- **한국 시간 2025-11-07 02:00** (새벽 2시)
  - UTC: 2025-11-06 17:00
  - 조회 결과: 2025-11-07에 추가된 곡들 ✅

- **한국 시간 2025-11-06 23:59** (자정 직전)
  - 조회 결과: 2025-11-06에 추가된 곡들

- **한국 시간 2025-11-07 00:00** (자정)
  - 조회 결과: 비어있음 (새로운 날 시작)
